...
function eventStoreHelper() {
  const helper = {
    ...
    getSnapshot: function(aggregateName, id) {
      return redis.hgetall(`AMS:${aggregateName}:${id}:snapshot`);
    },

    getEventsAfterSnapOffset: function(aggregateName, id, offset) {
      return redis.zrange(`AMS:${aggregateName}:${id}:events`, offset, -1);
    },

    getLastOffset: function(aggregateName, id) {
      let offset;
      return redis
        .zrevrange(`AMS:${aggregateName}:${id}:events`, 0, 0, "WITHSCORES")
        .then(result => {
