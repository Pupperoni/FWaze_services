...
function CommonAggregateHandler(eventStoreHelper) {
  const handler = {
    ...
    initialzeAggregateHandlers() {
      ...
      fs.readdir(`${process.cwd()}/aggregateHelpers/child`, (err, files) => {
        for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
          // get aggregate names from each file
          const handler = require(`${process.cwd()}/aggregateHelpers/child/${
            files[fileIndex]
          }`);
    ...
    getCurrentState(aggregateName, aggregateID) {
      let aggregateHandler = handler.aggregateHandlerList[aggregateName];
