...
const Handler = {
  ...
  getUserById(req, res, next) {
    // Redis get
    redis
      .hgetall(`UMS:user:${req.params.id}`)
      .then(result => {
        if (!result)
          return res
            .status(400)
        ...
        redis.hgetall(`UMS:user:${req.params.id}:home`).then(home => {
          if (home) result.home = home;
          // Then get work address
          redis.hgetall(`UMS:user:${req.params.id}:work`).then(work => {
  ...
  getFaveRoutes(req, res, next) {
    redis.hgetall(`UMS:user:${req.params.id}`).then(user => {
      if (!user)
  ...
  loginUser(req, res, next) {
    ...
    if (name && password && name !== "" && password !== "") {
      ...
      redis
        .get(`UMS:user:name:${name}`) // check if user exists
        .then(userId => {
          if (!userId)
            return Promise.reject(CONSTANTS.ERRORS.DEFAULT_LOGIN_FAILURE);
          id = userId;
          return redis.hgetall(`UMS:user:${id}`);
        })
        .then(result => {
          user = result;
        ...
        .then(isMatch => {
          if (isMatch) {
            // Get home address haha
            return redis.hgetall(`UMS:user:${id}:home`); // get home details
          } else return Promise.reject(CONSTANTS.ERRORS.DEFAULT_LOGIN_FAILURE);
        })
        .then(home => {
          if (home) user.home = home;
          // Then get work address
          return redis.hgetall(`UMS:user:${id}:work`); // and work details
        })
        .then(work => {
