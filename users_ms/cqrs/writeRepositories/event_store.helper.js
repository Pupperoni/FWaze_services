...
function eventStoreHelper() {
  const helper = {
    getSnapshotAndEvents: function(aggregateName, id) {
      ...
      return Promise.resolve(
        this.getSnapshot(aggregateName, id)
          .then(lastSnapshot => {
            console.log("[EVENTS STORE HELPER] Snapshot:", lastSnapshot);
    ...
    getSnapshot: function(aggregateName, id) {
      return redis.hgetall(`UMS:${aggregateName}:${id}:snapshot`);
    },

    getEventsAfterSnapOffset: function(aggregateName, id, offset) {
      return redis.zrange(`UMS:${aggregateName}:${id}:events`, offset, -1);
    },

    getLastOffset: function(aggregateName, id) {
      let offset;
      return redis
        .zrevrange(`UMS:${aggregateName}:${id}:events`, 0, 0, "WITHSCORES")
        .then(result => {
